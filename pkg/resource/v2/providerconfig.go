...
func (u *ProviderConfigUsageTracker) Track(ctx context.Context, mg Managed) error {
	...
	ref := mg.GetProviderConfigTypedReference()
	if ref == nil {
	...
	pcu.SetProviderConfigTypedReference(xpv1.TypedReference{
	...
	err := u.c.Apply(ctx, pcu,
		...
		resource.AllowUpdateIf(func(current, _ runtime.Object) bool {
			//nolint:forcetypeassert // Will always be a PCU.
			return current.(ProviderConfigUsage).GetProviderConfigTypedReference() != pcu.GetProviderConfigTypedReference()
		}),
	)
	return errors.Wrap(resource.Ignore(resource.IsNotAllowed, err), errApplyPCU)
}
